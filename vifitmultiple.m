...
clear;clc;
dataPath = 'D:\03 硕士第一阶段研究\数据收集与整理\rice_LAI_v14.xlsx';
OutPathName = 'D:\03 硕士第一阶段研究\结果图\05实测数据拟合结果\v10\04';
% 读取 Excel 数据
T = readtable(dataPath);
%% 02 提取对应时期的反射率
% 提取相关列（仅限符合筛选条件的行）
LAI = T{:, 31};             % 第29列为 LAI 实测数据
blue = T{:, 7} * 0.0001;   % 第7列为蓝波段反射率
green = T{:, 8} * 0.0001;  % 第8列为绿波段反射率
red = T{:, 9} * 0.0001;    % 第9列为红波段反射率
re1 = T{:, 10} * 0.0001;   % 第10列为红边1反射率
re2 = T{:, 11} * 0.0001;   % 第11列为红边2反射率
re3 = T{:, 12} * 0.0001;   % 第12列为红边3反射率
sw1 = T{:, 13} * 0.0001;   % 第13列为短波红外1反射率
sw2 = T{:, 14} * 0.0001;   % 第14列为短波红外2反射率
nir = T{:, 15} * 0.0001;   % 第15列为近红外反射率
number=length(LAI);
selectedRows1 = strcmp(T{:, 24}, '01'); % 字符串比较筛选符合当前时期编号的行
selectedRows2 = strcmp(T{:, 24}, '02'); % 字符串比较筛选符合当前时期编号的行
selectedRows3 = strcmp(T{:, 24}, '03'); % 字符串比较筛选符合当前时期编号的行
LAI1 = T{selectedRows1, 31};  
LAI2 = T{selectedRows2, 31};  
LAI3 = T{selectedRows3, 31};  
%%
%03 准备30种植被指数矩阵
%第一类
...
VI_struct(4).name='CI_{green}';VI_struct(4).formula=nir./green-1;
...
VI_struct(8).name='SAVI';VI_struct(8).formula=(nir-red)./sqrt(nir+red);
VI_struct(9).name='SWIRR';VI_struct(9).formula=sw1./sw2;
VI_struct(10).name='NDWI';VI_struct(10).formula=(nir-sw2)./(nir+sw2);
%第二类
VI_struct(11).name='CI_{re1}';VI_struct(11).formula=nir./re1-1;
VI_struct(12).name='CI_{re2}';VI_struct(12).formula=nir./re2-1;
VI_struct(13).name='CI_{3re}';VI_struct(13).formula=(re3-(0.3*re1+0.7*re2))./(0.3*re1+0.7*re2);
VI_struct(14).name='NDVI_{re1}';VI_struct(14).formula=(nir-re1)./(nir+re1);
VI_struct(15).name='NDVI_{re2}';VI_struct(15).formula=(nir-re2)./(nir+re2);
VI_struct(16).name='NDVI_{3re}';VI_struct(16).formula=(re3-(0.3*re1+0.7*re2))./(re3+(0.3*re1+0.7*re2));
...
VI_struct(19).name='MSR_{re}';VI_struct(19).formula=(nir./re1-1)./sqrt(nir./re1+1);
VI_struct(20).name='CIVI';VI_struct(20).formula=((nir - red) .* (nir - re1) .* (re2 - red))./((nir + red) .* (re2 - re1));
%第三类
VI_struct(21).name='RDVI';VI_struct(21).formula=1.5*(nir-red)./(nir+red+0.5);
...
VI_struct(28).name='SAVI_{green}';VI_struct(28).formula=1.5*(nir-green)./(red+green+0.5);
...
%% 提取不同生育时期的植被指数值
% 初始化不同时期的VI矩阵
VIsumvalue1 = zeros(sum(selectedRows1), num_indices); % 对应LAI1的VI值
VIsumvalue2 = zeros(sum(selectedRows2), num_indices); % 对应LAI2的VI值
VIsumvalue3 = zeros(sum(selectedRows3), num_indices); % 对应LAI3的VI值

% 遍历每个植被指数，计算值并分别存储
for i = 1:num_indices
    VI_all = VI_struct(i).formula; % 当前植被指数的公式
    
    % 根据筛选条件提取不同生育时期的值
    VIsumvalue1(:,i) = VI_all(selectedRows1); % 提取时期01的VI值
    VIsumvalue2(:,i)= VI_all(selectedRows2); % 提取时期02的VI值
    VIsumvalue3(:,i)= VI_all(selectedRows3); % 提取时期03的VI值
end
...
for idx=1:num_indices
    ...
    VIvalue1=VIsumvalue1(:,idx);
    VIvalue2=VIsumvalue2(:,idx);
    VIvalue3=VIsumvalue3(:,idx);
    %%
    %05 拟合形式以及初始参数设置
    % 2. 指数拟合
    ...
    logarithmic_start = [1, 10]; % 前两个时期的范围是：[1,5]
    % 4. 幂拟合
    ...
    fig = scattermultiple(R2,Bias,RMSE,RRMSE,VI_range,LAI_estimated,x_inition,x_radius,y_inition,y_radius,VI_name,number,LAI1,LAI2,LAI3,VIvalue1,VIvalue2,VIvalue3);
